...
class LossesContainer(Container):
    ...
    def __call__(
        ...
        """
        # raise ValueError(f"-----------------------------debug compiled_loss __call__1")
        y_true = self._conform_to_outputs(y_pred, y_true)
        # raise ValueError(f"-----------------------------debug compiled_loss __call__2")
        ...
        # raise ValueError(f"-----------------------------debug compiled_loss __call__3")
        ...
        # raise ValueError(f"y_true: {y_true}"
        #                  f"y_pred: {y_pred}"
        #                  f"sample_weight: {sample_weight}"
        #                  f"self._losses: {self._losses}"
        #                  f"self._loss_weights: {self._loss_weights}"
        #                  f"self._per_output_metrics: {self._per_output_metrics}")

        for y_t, y_p, sw, loss_obj, loss_weight, metric_obj in zip(*zip_args):
            # raise ValueError(f"-----------------------------debug compiled_loss __call__4.1")
            if (
