...
class Functional(training_lib.Model):
    ...
    def _insert_layers(self, layers, relevant_nodes=None):
        ...
        remove_layer_idx = []
        for idx, layer in enumerate(self._self_tracked_trackables):
            if str(type(layer)) == "<class 'tensorflow.python.trackable.data_structures.ListWrapper'>":
                remove_layer_idx.append(idx)
            if str(type(layer)) == "<class 'tensorflow.python.trackable.data_structures._DictWrapper'>":
                remove_layer_idx.append(idx)
                
        # self._self_tracked_trackables_new = []
        # for idx, layer in enumerate(self._self_tracked_trackables):
        #     if idx not in remove_layer_idx:
        #         self._self_tracked_trackables_new.append(layer)
        # self._self_tracked_trackables = self._self_tracked_trackables_new
        # print('self._self_tracked_trackables type: %s' % type(self._self_tracked_trackables[-2]))
        # print('self._self_tracked_trackables type: %s' % type(self._self_tracked_trackables[-1]))
        pop_count = 0
        while remove_layer_idx:
            pop_idx = remove_layer_idx.pop(0) - pop_count
            self._self_tracked_trackables.pop(pop_idx)
            pop_count += 1
        print('remove_layer_idx: %s' % remove_layer_idx)
        # print('self._self_tracked_trackables: %s' % self._self_tracked_trackables)
    ...
    def _graph_network_add_loss(self, symbolic_loss):
        ...
        # print('new_layers: %s' % new_layers)
        # print('new_nodes: %s' % new_nodes)
